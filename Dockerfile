...
ARG GO_MODULE="github.com/stuttgart-things/sweatShop-fetcher"
ARG VERSION=""
ARG BUILD_DATE=""
ARG COMMIT=""
...
RUN go mod tidy
RUN CGO_ENABLED=0 go build -o /bin/sweatShop-fetcher \
...
COPY --from=builder /bin/sweatShop-fetcher /bin/sweatShop-fetcher

ENTRYPOINT ["sweatShop-fetcher"]
